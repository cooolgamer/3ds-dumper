...
bool run_cmd(cmd_id id, u32 flags, char** argv, char* err_str) {
    ...
    else if (id == CMD_ID_BLINKLED) {
        if (!strcmp(argv[0], "red")) {
            SetNotificationLED(1000, COLOR_RED); // It's yellow for some reasons
        } else if (!strcmp(argv[0], "blue")) {
            SetNotificationLED(1000, COLOR_BLUE);
        } else if (!strcmp(argv[0], "yellow")) {
            SetNotificationLED(1000, COLOR_YELLOW);
    ...
    *end = '\0';
    ...
    while (ptr < end) {
        ...
        if (!result) { // error handling
            SetNotificationLED(1000, COLOR_RED); // tells the user a error occured as the screens are black
            if (syntax_error) // severe error, can't continue
                flags &= ~(_FLG('o')|_FLG('s')); // never silent or optional

            // commented because it will already show an error on the fsutil part
            /*
